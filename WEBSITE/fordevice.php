...
SELECT name,id,image,category FROM smartphone_telephones
	WHERE (id,category) IN (SELECT DeviceId,DeviceType FROM available_services WHERE ServiceId=? AND ServiceCategory=?) UNION
...
SELECT name,id,image,category FROM modem_networking
	WHERE (id,category) IN (SELECT DeviceId,DeviceType FROM available_services WHERE ServiceId=? AND ServiceCategory=?) 
EOS
                   );
$id = intval($_POST['id']);
